...
class Fabric:
    def __init__(self, basePath='', subpath='', arm=False):
        self.type = 'vanilla'
        self.base = basePath[:-1]
        self.subpath = '{}/{}'.format(self.base, subpath)
        if arm:
            self.type = 'arm'
        self.bin = '{}/bin/{}'.format(self.base, self.type)

    def newCryptoMaterial(self):
        command = "{}/cryptogen generate --config={}/crypto-config.yaml".format(self.bin, self.subpath)
        p = subprocess.Popen(command.split(), 
                            ...
                            close_fds=True, 
                            shell=False, 
                            cwd=self.subpath).wait()
        print("[+] 'CRYPTO-CONFIG' folder generated with certificates.")

    def newGenesisBlock(self):
        command = "{0}/configtxgen -configPath {1} -profile EtcdRaft -channelID net-sys-channel -outputBlock {1}/channel-artifacts/genesis.block".format(self.bin, self.subpath)
        p = subprocess.Popen(command.split(),
                            bufsize=-1, 
                            executable=None, 
                            stdin=None, 
    ...
    def newChannelConfiguration(self, profile, channelID):
        command = "{0}/configtxgen -configPath {2} -profile {1} -outputCreateChannelTx {2}/channel-artifacts/{3}.tx -channelID {4}".format(self.bin, profile, self.subpath, profile.lower(), channelID.lower())
        p = subprocess.Popen(command.split(),
                            bufsize=-1, 
                            executable=None, 
                            stdin=None, 
    ...
    def newAnchorUpdate(self, profile, consortium, channelID):
        for c in consortium:
            command = "{0}/configtxgen -configPath {2} -profile {1} -outputAnchorPeersUpdate {2}/channel-artifacts/Org{3}MSPanchors_{4}.tx -channelID {4} -asOrg Org{3}MSP".format(self.bin, profile, self.subpath, c, channelID.lower())
    ...
    def instantiateNetwork(self, orgs, domain):
        for o in range(orgs):
            files = os.listdir('{}/crypto-config/peerOrganizations/org{}.{}/ca'.format(self.subpath, o+1, domain))
    ...
    def createChannel(self, org, domain, ports, channelName, channelID):
        ...
        cli = client.containers.get('cli.{}'.format(domain))
